(() => {
  ...
  // const storageName = 'hexo-encrypt-token:#' + window.location.pathname;
  const storageName = 'hexo-encrypt-token';
...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  const mainElement = document.getElementById('hexo-encrypt-token');
  ...
  async function decrypt(decryptKey, iv, hmacKey) {
    ...
    const result = await cryptoObj.subtle.decrypt({
      ...
      document.getElementById('hexo-encrypt-token').style.display = 'inline';
      document.getElementById('hexo-encrypt-token').innerHTML = '';
      document.getElementById('hexo-encrypt-token').appendChild(await convertHTMLToElement(decoded));

      // 取消 再次加密 按钮
      // document.getElementById('hexo-encrypt-token').appendChild(hideButton);
  ...
  window.het = {};
  window.het.keydownDecrypt = keydownDecrypt;
  ...
  window.het.hbeLoader = hbeLoader;
